...
struct HomeView: View {
    ...
    var body: some View { ScrollView { VStack(alignment: .leading, spacing: 24) { VStack(alignment: .leading, spacing: 8) { Image(systemName: "water.waves").font(.system(size: 48)).foregroundStyle(Color.appPrimary); Text("Dive ID").font(.largeTitle.bold()); Text("Describe the marine life you saw and review possible matches from your offline regional pack.").foregroundStyle(.secondary) }
        ...
        else if viewModel.catalogueLoaded { EmptyStateView(title: "No reviewed catalogue available", message: "No human-reviewed identification catalogue is currently available for new searches. You can still open Saved Identifications below.") }
        FeatureActionCard(title: "Describe What You Saw", subtitle: viewModel.canSearch ? "Use color, shape, size, and habitat clues" : "Unavailable until a reviewed catalogue is included", symbol: "text.alignleft") { if viewModel.canSearch { router.navigate(to: .descriptionSearch) } }.disabled(!viewModel.canSearch).accessibilityIdentifier("describeAction")
