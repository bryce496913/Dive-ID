...
final class HomeViewModel {
    ...
    var availablePacks: [OfflineIdentificationPackMetadata] = []
    var selectedRegion: OfflineIdentificationPackID?
    ...
    var canSearch: Bool { pack != nil }
    var canChangeRegion: Bool { !availablePacks.isEmpty }
    var selectedRegionUnavailable: Bool { catalogueLoaded && pack == nil && canChangeRegion }
}
...
struct HomeView: View {
    ...
    var body: some View { ScrollView { VStack(alignment: .leading, spacing: 24) { VStack(alignment: .leading, spacing: 8) { Image(systemName: "water.waves").font(.system(size: 48)).foregroundStyle(Color.appPrimary); Text("Dive ID").font(.largeTitle.bold()); Text("Describe the marine life you saw and review possible matches from your offline regional pack.").foregroundStyle(.secondary) }
        ...
        else if viewModel.selectedRegionUnavailable { EmptyStateView(title: "Selected region unavailable", message: "The selected dive region has no reviewed catalogue available for new searches. Choose another available region to continue.") }
        else if viewModel.catalogueLoaded { EmptyStateView(title: "No reviewed catalogue available anywhere", message: "No human-reviewed identification catalogue is currently available for new searches. You can still open Saved Identifications below.") }
        if viewModel.pack == nil && viewModel.canChangeRegion {
            Button { router.navigate(to: .offlineRegions) } label: { Label("Change Dive Region", systemImage: "map.fill").frame(minHeight: 44) }
                .accessibilityIdentifier("changeRegionAction")
        }
        FeatureActionCard(title: "Describe What You Saw", subtitle: viewModel.canSearch ? "Use color, shape, size, and habitat clues" : "Unavailable for the selected dive region", symbol: "text.alignleft") { if viewModel.canSearch { router.navigate(to: .descriptionSearch) } }.disabled(!viewModel.canSearch).accessibilityIdentifier("describeAction")
    ...
    }.padding() }.appScreenBackground().navigationTitle("Home").task(id: router.path) { await viewModel.load() } }
}
