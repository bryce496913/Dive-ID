...
struct OfflineRegionsView: View {
    ...
    var body: some View { List { if let error = viewModel.errorMessage { Text(error).foregroundStyle(Color.appError) }
        ...
        Text("More regional packs may be added later. The Caribbean and Tropical Pacific packs are included with this version and work without internet access.").font(.footnote).foregroundStyle(.secondary)
